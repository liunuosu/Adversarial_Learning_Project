...
class adversary_model(tf.Module):
    ...
    def forward(self, classifier_logits, true_income):
        ...
        # hidden_layer = tf.nn.dropout(hidden_layer, rate=0.2, seed=self.seed3)
...
class GeneralAdversary_validation(BaseEstimator, ClassifierMixin):
    def __init__(self, prot_attr=None, scope_name='classifier',
                 adversary_loss_weight=0.1, num_epochs=50, batch_size=256, random_state=None):
    ...
    def fit(self, x_train, y_train, x_validation, y_validation):
        ...
        starter_learning_rate_classifier = 0.0025
        ...
        for epoch in range(self.num_epochs):
            ...
            for i in range(num_train_samples // self.batch_size):
                ...
                if i % (num_train_samples // self.batch_size - 1) == 0 and i != 0:
                    ...
                    print(
                        "(TRAINING  ) epoch %d; clf loss: %f; adv loss: %f; classifier accuracy: %f; "
                        "DAO: %f; FPRf: %f; FPRm: %f; FNRf: %f ; FNRm: %f adversary accuracy: %f;" % (
                            epoch + 1, loss_classifier, loss_adv, clf_accuracy, batch_dao, p1, p2, p1_n, p2_n, adv_accuracy))
...
class EarlyStopper:
    ...
    def early_stop_adv(self, validation_loss, epoch, clf_accuracy, DAO):
        ...
        if DAO < self.DAO and clf_accuracy > 0.84:
            self.min_validation_loss = validation_loss
            self.counter = 0
            self.epoch = epoch
            self.clf_accuracy = clf_accuracy
